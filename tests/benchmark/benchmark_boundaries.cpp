#include <benchmark/benchmark.h>
#include <cuda_runtime.h>

#include "simulation.h"
#include "types.h"

class BoundariesFixture : public benchmark::Fixture {
   public:
    Simulation* sim = nullptr;
    MarketParams params;

    void SetUp(const ::benchmark::State& state) override {
        params.num_agents = state.range(0);
        params.num_steps = 100;
        params.time_delta = 1.0f / 60.0f;
        params.price_init = 100.0f;
        params.price_randomness_stddev = 0.1f;
        params.permanent_impact = 1e-5f;
        params.temporary_impact = 0.01f;
        params.sph_smoothing_radius = 1.0f;
        params.congestion_sensitivity = 0.05f;
        params.hash_table_size = params.num_agents;
        params.decay_rate = 0.0001f;
        params.mass_alpha = 0.4f;
        params.mass_beta = 0.1f;
        params.risk_mean = 0.01f;
        params.risk_stddev = 0.1f;

        // We don't need external buffers for this test
        sim = new Simulation(params);

        // Run one step to populate data
        sim->step();
    }

    void TearDown(const ::benchmark::State& state) override { delete sim; }
};

BENCHMARK_DEFINE_F(BoundariesFixture, GetBoundaries)(benchmark::State& state) {
    for (auto _ : state) {
        Boundaries b = sim->getBoundaries();
        benchmark::DoNotOptimize(b);
        cudaError_t err = cudaGetLastError();
        if (err != cudaSuccess) {
            state.SkipWithError(cudaGetErrorString(err));
            break;
        }
    }
}

BENCHMARK_REGISTER_F(BoundariesFixture, GetBoundaries)
    ->RangeMultiplier(10)
    ->Range(1000, 1000000)
    ->Unit(benchmark::kMillisecond);
